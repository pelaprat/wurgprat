...
export default function IngredientsPage() {
  ...
  const handleAutoAssignDepartments = async () => {
    setIsAutoAssigning(true);
    try {
      const response = await fetch("/api/ingredients/auto-departments", {
        method: "POST",
      });
      if (response.ok) {
        const data = await response.json();
        ...
        const data = await response.json();
        alert(data.error || "Failed to auto-assign departments");
      }
    } catch (error) {
      console.error("Failed to auto-assign departments:", error);
      alert("Failed to auto-assign departments");
    } finally {
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button
            onClick={handleAutoAssignDepartments}
            disabled={isAutoAssigning}
            className="px-4 py-2 text-sm bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50 transition-colors flex items-center"
          >
            {isAutoAssigning ? (
              <>
